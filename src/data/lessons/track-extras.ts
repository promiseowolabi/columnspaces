 ...
 * side note for each of the ten tracks. Hand-written, and deliberately NOT in
 * the generated manifest — a generator must never own copy.
 *
 * Split out of index.ts so the curriculum and track pages can read it without
 * importing the 54-lesson corpus.
 */

import type { LucideIcon } from 'lucide-react'
import { Warehouse } from 'lucide-react'
import type { SimId, TrackId } from './types'
import { metaForTrack, type LessonMeta } from './manifest'
...
export function simsForTrack(trackId: string): { sim: SimInfo; lesson: LessonMeta }[] {
  const out: { sim: SimInfo; lesson: LessonMeta }[] = []
  const seen = new Set<SimId>()
  for (const l of metaForTrack(trackId)) {
