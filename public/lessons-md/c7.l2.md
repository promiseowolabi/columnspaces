# C7.L2 — JSON and the Schema You Did Not Declare

_Track C7: Beyond Flat Columns · ~17 min · columnspaces_

> A JSON column stored as text is one byte range per row group and zero pruning. Shred four fields out of it and you get statistics that skip files — until one row arrives with a string where the timestamp was.
Somewhere in every analytical platform there is a column that holds JSON. It is called `properties`, or `payload`, or `attributes`, or `raw`. It exists because at the moment it was created nobody could enumerate the fields, and because the producer wanted to ship without a schema review.
...
## What the blob actually costs you
...
> **[vendor snapshot — verified 2026-09]** How Parquet and Iceberg now specify semi-structured columns
>
> The "store both" answer is now written down. Parquet's `VARIANT` logical type annotates a group with a required `metadata` binary field and a `value` binary field; the shredded form adds a `typed_value` field beside `value`:
> 
> ```text
...
>
> Sources: https://github.com/apache/parquet-format/blob/master/VariantShredding.md · https://github.com/apache/parquet-format/blob/master/LogicalTypes.md · https://iceberg.apache.org/spec/

---

## The honest cost: a shredded column is a schema decision wearing a flexibility costume
...
_fig 1 — the same JSON column, three physical layouts, and the one condition that revokes the fast path_

Components: 2e9 events · ~400 B of JSON each · queries filter on 2 of ~40 fields (nobody could enumerate the fields when the column was created, and that has not changed) · stored as text (cheap write · 0% pruning) · shredded to typed_value (prunable · a schema decision) · value AND typed_value (the partially shredded object) · ≈160 GB read for a 2-field filter (you cannot project inside an opaque value) · ≈0.06 GB read, ~99% pruned (per-chunk min/max on 4 real columns) · one row arrives with a string where the timestamp was (it lands in value, not typed_value — nothing fails, nothing is logged, the row still reads correctly) · skipping is no longer valid (the spec allows it only while value is all-null) · shred the predicates, version the set (and alert on the fallback count from day one)

Steps:
1. Two billion events, four hundred bytes of JSON each, and a dashboard that filters on two fields out of roughly forty. This is the most common shape in any event platform, and it is the shape the format was worst at until recently.
2. Stored as text the write path is trivial and the read path is a full scan: statistics describe the bytes of the serialised document, which answers no predicate anybody writes, so the pruning ratio is exactly zero.
3. Shred the two filtered fields into real typed columns and the whole apparatus from C2 applies again — per-chunk minima and maxima, dictionary encoding, and a projection that touches two byte ranges instead of every document.
4. So the engineering answer is both at once: a value field for whatever was not shredded and a typed_value field for what was. Parquet writes them side by side, and their four null combinations carry four distinct meanings.
5. Then the shape drifts. One producer sends a date string where you shredded a microsecond timestamp. It lands in the fallback field, no job fails, and the row reads back correctly — which is exactly why nobody notices.
6. And the fast path is revoked by specification, not by bug: statistics on the shredded column may only be used for skipping while the fallback is always null. The pruning ratio you promised changes with no deployment.
7. Which leaves three obligations rather than a recommendation: shred only the paths that appear in predicates, version that set as a published contract, and alert on the fallback count — it moves on day one of the drift.

---

- **0%** — pruning on a JSON column stored as text (Statistics describe the serialised bytes of the document, not the fields inside it. No predicate on a JSON path can skip a row group.)
- **4 of ~40** — fields worth shredding in a typical event payload (The ones that appear in WHERE clauses. Shredding all forty costs storage on sparse columns and buys nothing the queries use.)
- **4 combinations** — of value and typed_value, all meaningful (Both null is missing; value only is present-any-type; typed_value only is present-and-typed; both is a partially shredded object.)
- **1 row** — of drift is enough to revoke skipping on that column (The Parquet shredding spec permits skipping on typed_value only while the paired value column is always null. One off-type value populates it.)

---

> **[warning]** `SELECT count(*) FROM events WHERE json_extract(payload, '$.plan') = 'enterprise'` is a full table scan of the payload column with a JSON parse per row, and it will be reported in a dashboard as a slow query rather than as an expensive one. Two things hide the cost:
> 
...
_shredding ≡ decisions you have already made under another name_

- **a generated column with an index, over a JSONB field** (Postgres users extract the hot path into a stored generated column and index that, precisely because you cannot index inside an opaque document usefully. Everyone accepts it as a schema commitment.) ≡ **a shredded typed_value column** (The same move one layer over, with per-chunk statistics playing the part of the index. Same commitment, but usually made in a writer config rather than in DDL — so nobody reviews it.)
- **a schemaless document store with secondary indexes** (You get write flexibility, and then you add an index per query pattern and discover that the set of indexes is your schema, enumerated by hand and drifting from the documents.) ≡ **the shredded field set** (Identical structure and identical failure: the set of shredded paths is the real schema, it is not versioned with the table, and consumers cannot see it.)
- **a cache with an unmonitored miss path** (It works beautifully until the hit rate quietly falls, and because the miss path is correct rather than broken, the only symptom is cost.) ≡ **the value fallback field** (A correct, silent, slow path that is entered by data rather than by deploy — which is why the fallback count is the metric to alert on, not the error rate.)

---

**Q1. A JSON payload column is shredded on event_ts as an int64 timestamp. Three weeks later, one upstream service begins sending event_ts as an ISO date string. What is the state of the table?**
   A. Ingest fails for those rows and they are quarantined, so the table is unaffected
   B. Rows still read back correctly — the off-type values land in the value field — but the engine may no longer use typed_value statistics to skip data for the affected files, so a dashboard slows down with no code or config change anywhere
   C. The typed_value column is corrupt and the affected files must be rewritten before queries are correct
   D. The values are coerced to timestamps on write, so the only risk is a parsing error on malformed dates
   Answer: B — This is the shredding design working exactly as specified: the fallback field exists so that an off-type value is stored losslessly rather than rejected. The consequence is in the skipping rule — statistics on a shredded column may be used for file, row-group or page skipping only while the paired value column is always null, because only then is the type known for every row. Nothing is corrupt and nothing is quarantined; correctness is preserved and the cost model is not, which is the hardest class of regression to attribute because no change was deployed.

**Q2. A team proposes shredding all 38 fields found in a JSON payload column, "so every future query is fast". What is the strongest objection?**
   A. It will not work, because Parquet limits the number of shredded fields per variant column
   B. Most of those fields appear in no predicate and are null on most rows, so you pay write cost and per-column footer overhead across every file for statistics nobody consults — and you have quietly turned the entire payload into a declared schema, which means every future producer change is now a schema change on your table
   C. Shredding is only valid for scalar fields, so nested objects in the payload cannot be shredded at all
   D. It is the right call: storage is the cheapest term in the model, so shredding everything strictly dominates
   Answer: B — Two costs compound. The footer cost is the one from C3.L1 — statistics entries scale as row groups times leaf columns, and each entry quotes real minimum and maximum values — so 38 sparse leaves multiply the metadata on every file the loader writes, including files nothing queries. The larger cost is organisational: the whole reason the column was JSON was to absorb producer change without a schema review, and shredding all of it puts that change back in the review path while leaving it invisible in the table definition. Nested objects and arrays can be shredded (the spec defines both), and there is no per-column limit at issue here; the fourth option ignores that the scan bill, not storage, is usually the larger line.

**Q3. You are asked in a design review to defend "shredding four fields cuts our dashboard scan from about 160 GB to about 0.06 GB". Which answer survives the room?**
   A. "Those are measured numbers from our production cluster, so the improvement is confirmed."
   B. "The ratio comes from projection and pruning, which are mechanisms I can show you — but it holds only while every value of those four fields matches the type we shredded, and the format only permits skipping while the fallback field is empty. So I want two things in the proof of concept: the pruning ratio on our real query mix, and the count of rows landing in the fallback per field per day. If that second number is not zero, the first number is not what I quoted."
   C. "It is a conservative estimate; the real improvement will be larger once caching warms up."
   D. "We should not commit to a figure until we have run a full benchmark against the alternatives."
...
**`VariantEncoding.md` and `VariantShredding.md`** in `parquet-format` are the primary sources and are short enough to read in a sitting. The shredding document's reconstruction pseudocode is the part worth typing out — it makes the four-way null table concrete, and the table of INVALID cases at the end tells you exactly which writer bugs to expect from an immature implementation.
