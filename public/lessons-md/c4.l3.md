# C4.L3 — Late Materialization

_Track C4: Vectorized Execution · ~15 min · columnspaces_

> Forge lab 03's storm scans 37,915 rows through two chained predicates and materialises exactly 4,871 values — one per survivor, once per batch, at the end. The other 33,044 rows were never assembled into anything.
A row is a convenience for humans. An aggregate does not need one.
...
```rust
/// `keys` and `vals` are two columns of the SAME batch, so a position indexes
...
## What the lab actually grades, by check id
...
_fig 1 — four columns, two predicates, one aggregate, and no row anywhere_

Components: select region, sum(amount) where ts in window and status = paid (four columns touched · one group-by · zero rows assembled) · ts (predicate 1) · status (predicate 2) · amount (aggregated) · region (group key) · Sel after the time window (positions only — no values copied out of ts, and the other three columns are untouched) · Sel after status = paid  ->  4,871 of 37,915 rows (12.8%) (the second predicate read status only at the surviving positions) · fold amount[pos] into group region[pos] (225 groups accumulated across batch boundaries · 0 tuples built) · materialise: 4,871 values (once per batch, at the end, and only for what a client will see) · where it loses: many projected columns · high selectivity · scattered positions

Steps:
1. A dashboard query over four columns. A row store would assemble every candidate record and discard most of the bytes; a column store need not assemble anything at all, and this is the sequence in which it does not.
2. The time-window predicate reads only the ts column and produces positions. Nothing has been copied, and status, amount and region have not been touched — projection and late materialization are the same economy applied at two granularities.
3. The status predicate reads status at the surviving positions and narrows the same list. In the lab storm this leaves 4,871 of 37,915 rows, and the second column was never read for the 33,044 rows already eliminated.
4. Now the aggregate. Region and amount are two columns of the same batch, so one position indexes both, and each survivor folds a value into a group without the pair ever being written to memory. 225 groups, accumulated across batch boundaries in one table.
5. Materialisation happens once, at the end, and only for values something outside the engine will see: 4,871 copies, equal to the number of survivors. The ratio to rows scanned is the query's selectivity, not a property of the engine.
6. And the honest half: three situations invert this. Projecting a dozen columns means a dozen scattered gathers; high selectivity means you were going to copy nearly everything anyway; and a position that means "row 4,912 of a compressed page" is a seek, not an offset.

---

## Three places late materialization loses
...
- **4,871 / 37,915** — values materialised versus rows scanned (storm) (One copy per survivor, once per batch, at the end of the chain. The ratio is the query's selectivity — 12.8% here — not an engine constant.)
- **1,219 of 2,000** — graded batches where the selection survived nothing (An empty selection must produce no values and no groups. In the storm it happens on more than half the batches, which is why it is the edge case that carries the grade.)
- **225** — groups accumulated across batch boundaries (One GroupTable per query, not per batch. A key seen in batch 0 and batch 7 is one group with one running aggregate.)
- **16 → 2** — pages a gather touches, scattered versus clustered (131,072-row row group, 8,192-row pages, 12.8% selectivity. Late materialization saves page reads only when the survivors are physically near each other — which is a C2 layout property.)

---

_late materialization ≡ deferrals you already reason about_

- **index-only scan, then heap fetch** (If the index covers the query, the heap is never touched. If it does not, you pay a random fetch per matching row — and past a few percent selectivity the planner switches to a sequential scan for exactly that reason.) ≡ **positions, then gather** (The same crossover, and the same planner decision: below a few percent the position list wins, above roughly a third of the batch you should have materialised and read dense arrays.)
- **lazy loading, and the N+1 problem** (Deferring the fetch is free until something iterates the collection, at which point you have issued one query per element and the deferral has become the bug.) ≡ **a gather per projected column** (Deferring the copy is free until twelve expressions each read the survivors through the selection. Then you have paid twelve scattered passes to avoid one copy.)
- **a pointer into a page cache** (An offset is cheap while the page is resident and expensive the moment it is not — which is why "just keep a reference" is a claim about the cache, not about the pointer.) ≡ **a position into a column chunk** (Cheap while it means an array offset, expensive when it means locate the page, decompress it, and follow a dictionary code. The lab can only show the cheap case; production supplies the other one.)

---

**Q1. An ad-hoc query does SELECT * FROM events WHERE ts BETWEEN ... AND status = 'paid', returning about 40% of a row group. Your engine uses selection vectors and gathers each projected column at the end. Where is the cost going?**
   A. Into the two predicates, since both must be evaluated over the whole row group
   B. Into the gathers: with 24 projected columns and 40% selectivity you are doing 24 scattered passes to avoid copies you were going to make anyway — this is the shape where compacting the survivors once and reading dense arrays is cheaper, and late materialization has almost nothing to offer
   C. Into the group table, because SELECT * forces every column into the aggregate state
   D. Into the selection vector's memory, which at 40% selectivity exceeds the size of the batch itself
   Answer: B — Late materialization defers one copy per column, so its value scales inversely with the number of projected columns and with selectivity. Twenty-four columns means twenty-four consumers of the survivor list, which is well past the roughly-two-consumer crossover, and 40% selectivity means the copies avoided are a minority of the batch. Both effects point the same way: compact once, then read dense arrays. The predicates are not the problem — the second one is evaluated only over the first one's survivors. And a Vec<u32> at 40% selectivity is about 1.6 bytes per row of the batch, which is real but not the dominant term.

**Q2. Two tables have identical schemas, identical row counts and identical queries. One is written in event-time order; the other arrives shuffled. Both use late materialization. Why does only one of them get the benefit?**
   A. It does not depend on write order — late materialization is an execution technique and works identically on any physical layout
   B. Because positions resolve to pages: with survivors clustered into 2 of 16 pages the gather reads 2 pages, while uniformly scattered survivors at the same selectivity touch all 16 — so the deferral saves page reads only when the layout puts survivors near each other
   C. Because the shuffled table has more distinct values, so the dictionary is larger
   D. Because the shuffled table cannot use selection vectors, only bitmaps
   Answer: B — In a lab, a position is an array offset and a gather is a memory read. In an engine, a position may mean row 4,912 of a compressed, encoded page — so the count that matters is pages touched, not values copied. At 12.8% selectivity, uniformly scattered survivors touch every page of the chunk and the deferral saved nothing while still costing an indirection per value; clustered survivors touch two. That makes the payoff of late materialization a property of the physical layout, which is C2 reappearing inside the execution engine, and it is the reason clustering depth belongs on a dashboard rather than in a design document.

**Q3. Forge lab 03's aggregate_correct includes a case with four rows of one group holding 1, 100, 2, 200, where only positions 0 and 2 are selected. The expected answer is sum 3 and max 2. What class of bug is that case built to catch, and why is it graded group by group rather than on totals?**
   A. Integer overflow in the accumulator, which is why sums are i128
   B. An aggregate that walks the batch instead of the selection — it silently includes rows a predicate already rejected and reports a plausible number, and per-group comparison is required because two groups can be wrong in opposite directions while the total looks correct
   C. A null-handling bug, since unselected rows behave like nulls
   D. An ordering bug, since the output must be sorted by key with NULL first
