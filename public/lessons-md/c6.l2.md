# C6.L2 — Broadcast or Partition

_Track C6: Distributed Analytics · ~16 min · columnspaces_

> Broadcast moves the small side once per worker; partitioning moves both sides once. The crossover is exactly 1 + probe/small, which means the plan that was right last quarter is arithmetic away from being wrong this one.
There are two ways to make matching rows meet, there is no third one, and the choice between them is arithmetic rather than taste.
...
## Three joins, and the arithmetic each one forces
...
_fig 1 — clicks ⋈ campaigns: two plans, two questions, two different winners_

Components: clicks ⋈ campaigns (1.6 GiB build · 47 GiB probe · 32 workers) · broadcast: 1.6 GiB × 32 (50 GiB moved · each worker holds all 1.6 GiB) · partitioned: 1.6 + 47 (48.6 GiB moved · each worker holds its slice) · question 1 — total bytes moved: partitioned, by 3% (crossover is 31 workers, and the cluster has 32) · broadcast busiest worker: 3.0 GiB (skew factor exactly 1 — the probe side never re-routes) · partitioned busiest worker: 19.4 GiB (40% hot key × 32 workers = skew factor 12.80) · question 2 — busiest worker: broadcast, by 6.41× (same statistics, different question, opposite answer) · one join of three flips (and one has real skew that changes nothing — so "skew means broadcast" fails)

Steps:
1. One join, three statistics: a 1.6 GiB build side, a 47 GiB probe side and 32 workers. Everything that follows is two multiplications on those three numbers, and no planner opinion is required.
2. Broadcast sends the build side to every worker: 1.6 GiB times 32 is 50 GiB on the wire, and every worker must hold the whole 1.6 GiB in memory to probe against it. Partitioning moves each side exactly once: 48.6 GiB.
3. On total bytes the partitioned plan wins by 3%, which is close enough that the crossover matters: broadcast is cheaper up to 31 workers, and this cluster has 32. One more worker next quarter does not change the answer; one fewer would have.
4. Now ask what the busiest worker receives, because that is what the job waits on. Broadcast: the build side plus an even 1/32 of the probe side, 3.0 GiB, with a skew factor of exactly 1 because probe rows are never re-routed by the key.
5. Partitioning: the hot campaign holds 40% of the rows, so its partition receives 40% of 48.6 GiB — 19.4 GiB, a skew factor of 12.80 against the mean. Skew changed no byte total at all; it multiplied one term of one plan.
6. So the two questions have opposite answers on the same statistics, and this join is the one of three where that happens. The discipline is to ask both — total bytes for the bill, busiest worker for the wall clock.

---

- **239 vs 8** — crossover worker count, 180 MiB build against 9 GiB build (Both from 1 + probe/small on real probe sides of 42 and 64 GiB. Same query shape; the plan that is right differs by a factor of thirty in cluster size.)
- **1.00** — broadcast skew factor, always (Every worker receives the whole build side and reads an even slice of the probe side, so the busiest and mean workers are the same number regardless of key distribution.)
- **12.80** — partitioned skew factor at a 40% hot key over 32 workers (hotShare × workers. Skew leaves the byte total untouched and multiplies only the busiest-worker term, which is why a plan can be cheaper and slower at once.)
- **1 of 3** — joins where the decision flips once the key is known (Pinned by the lab test suite. Exactly one flips and exactly one has genuine skew where the plan does not change, so "skew means broadcast" cannot pass.)

---

> **[info]** Every layout and every plan privileges something and starves something else, so name what broadcast is bad at:
> 
...
**Browser lab: The Shuffle Planner** — Cost broadcast against partitioned for a join, then re-cost it when the key turns out to be skewed. (interactive, on the lesson page)

---

## What the lab grades, and what it cannot
...
_broadcast versus partition ≡ decisions you have already made_

- **replicating a lookup table to every application server** (Fine while the table is small and the fleet is small. The cost is the table times the fleet, and it grows when either does — which is why it is always discovered during a scale-out.) ≡ **a broadcast join** (Identical arithmetic: small × workers. The crossover is where the fleet gets big enough that shipping one copy each costs more than routing the requests.)
- **a covering index versus a sort-merge over both tables** (One puts the small thing where the big thing already is; the other reorganises both. The choice is selectivity arithmetic, not preference.) ≡ **broadcast versus partitioned** (The same shape at cluster scale, with the ratio of the two sides in place of selectivity — and the same failure mode when the ratio changes and nobody re-runs the sum.)
- **a CDN edge cache versus routing by hash** (Replicate to every edge, or send each key to the one node that owns it. Replication is even and expensive; hashing is cheap and vulnerable to hot keys.) ≡ **the two plans, and the skew asymmetry** (Broadcast has a skew factor of exactly 1 and a total that scales with the fleet; partitioning has a flat total and a busiest worker multiplied by hotShare × workers.)

---

**Q1. A join has a 6 GiB build side and a 120 GiB probe side, currently running well as a broadcast on 16 workers. Capacity planning wants to move to 64 workers to cut the nightly window. What happens to this join, and what do you say in the meeting?**
   A. Nothing — broadcast cost depends on the build side, which is unchanged, so the join scales with the cluster like everything else
   B. It becomes the wrong plan: the crossover is 1 + 120/6 = 21 workers, so at 64 workers broadcast moves 384 GiB against 126 GiB, and each worker still has to hold the whole 6 GiB build side in memory
   C. It gets faster in proportion to the workers added, because each worker processes a smaller slice of the probe side
   D. It depends on the key distribution, which has to be sampled before anything can be said
   Answer: B — Broadcast is the one plan whose total cost is proportional to the cluster, so a scale-out is exactly the event that invalidates it. The crossover is arithmetic available before the change: 1 + probe/small = 21 workers, and the proposal is 64. At 64 workers broadcast moves 6 × 64 = 384 GiB against 6 + 120 = 126 GiB partitioned, so the plan that was three times cheaper becomes three times more expensive. The memory point matters too — 6 GiB of build side per worker is a real constraint that partitioning reduces to a slice. Sampling the key is the right instinct for the second question, but it cannot rescue a plan that has already lost the first one by 3×.

**Q2. A join is 3% cheaper on total bytes as a partitioned shuffle. Sampling then reveals one key holds 35% of the rows over 32 workers. Which statement is correct?**
   A. Skew raises the total bytes moved by the partitioned plan, so broadcast is now cheaper on both counts
   B. Skew leaves both byte totals exactly unchanged and multiplies only the partitioned plan's busiest worker, by hotShare × workers = 11.2 — so the plan can be cheaper on the bill and much slower on the clock, and which one you optimise is a stated choice
   C. Skew always means broadcast, since broadcast has no partitioning step to be skewed
   D. The hot key should be filtered out and handled by a separate query, which is the only correct fix
   Answer: B — Byte totals are conservation statements: partitioning moves both sides once whatever the distribution, and broadcast moves small × workers whatever the distribution. Skew changes neither. What it changes is the busiest-worker term of the partitioned plan, multiplying it by hotShare × workers — 0.35 × 32 = 11.2 here — while leaving broadcast at a skew factor of exactly 1. So this join probably does flip on the runtime question, but "skew means broadcast" is still wrong as a rule: the lab has a join with a genuinely hot key at 6% where the threshold is 15.1% and the partitioned plan remains correct on both questions. The threshold is (small + probe/w) / (small + probe), and it is one division. Splitting the hot key out is a legitimate third option — that is salting, and C6.L3 prices it.

**Q3. You inherit a warehouse where every large join carries a broadcast hint added two years ago. What is the cheapest reliable audit, given a catalog with table sizes and a known worker count?**
   A. Remove all the hints and let the optimiser decide, since modern optimisers have better statistics than the original authors did
   B. For each hinted join compute probe / (workers − 1) and compare it against the build side: any join whose build side exceeds that ratio is moving more bytes than a shuffle would, and the list is produced from catalog metadata without running a query
   C. Run each join both ways in a staging cluster and keep the faster plan
   D. Sort the joins by table size and re-examine the ten largest, since the cost of a wrong plan scales with data volume
