# C2.L5 — The Small File Problem

_Track C2: Layout & Pruning · ~17 min · columnspaces_

> Planning took 61% of query time on a 9 TB table. The data had not grown. There were 1.4 million files where there used to be thirty-five thousand.
...
## Planning scales with file count, not with data size
...
## Creation rate versus compaction rate: there is no steady state to find
...
## The floor: partition boundaries are merge barriers
...
_fig 1 — the file count is a product, and one of its factors is a wall_

Components: 30-second commits (2,880 per day) · × partitions written (day × tenant × region) · × parallel writers (each emits its own file) · = 9,600 files/hour (120/hour before the change · avg 240 MB → 6 MB) · planning enumerates files (1.4M live · 61% of query time) · compaction merges files (5,000/hr against 9,600/hr → 940k backlog) · a partition boundary is a merge barrier (no file may span two partitions, so the floor is per-partition data) · so: P ≤ ingest ÷ target size (25 GB/day ÷ 512 MB = 48) · and the interval is the dial (freshness sets the file count, not compaction)

Steps:
1. Three factors, owned by three different people: the commit interval belongs to the freshness requirement, the partition count to the layout design, the writer count to the ingest job. Nobody owns the product.
2. Multiply them and the file count is the answer. Moving from 15-minute to 30-second batches took creation from 120 files an hour to 9,600 and dragged the average file from 240 MB down to 6 MB, on identical data.
3. The read side pays first and invisibly. Planning does bounded work per file regardless of file size, so 1.4 million files means 1.4 million metadata decisions — and bytes-scanned, the metric on the dashboard, does not move at all.
4. Compaction is rate-matching, not tidying: merging 5,000 files an hour against a creation rate of 9,600 gives a deficit of 4,600 an hour, and a deficit accumulates without bound. The observed 940,000-file backlog is about eight and a half days of it.
5. And a partition boundary is a wall the merge cannot cross, because a file belongs to exactly one partition and that is what makes partition pruning free. So the biggest file achievable in a partition is bounded by the data inside it.
6. Which yields both fixes as arithmetic rather than taste: cap partition cardinality at daily ingest divided by target file size, and treat the batch interval — not the compaction cluster — as the variable that sets the file count.

---

- **240 MB → 6 MB** — average file size, from one config change (Column Week INC-2. The commit interval went from 15 minutes to 30 seconds; the data volume did not change at all.)
- **61%** — of query time spent planning (Up from 4%. Planning does bounded work per file, so it scales with file count and is invisible to a bytes-scanned dashboard.)
- **48** — actively-written partitions the ingest volume supports (25 GB/day ÷ 512 MB target. Past this the files are structurally small and no compaction policy can fix it.)
- **1.4 TB/day** — rewrite needed to keep up on a 9 TB table (9,600 files/hour × 6 MB, read and written. Compaction that keeps pace rewrites ~15% of the table daily with no user-visible product.)

---

> **[warning]** Bytes scanned — the number C0 spent five lessons teaching you to defend — **does not move when the file count explodes.** Same data, same predicates, same bytes, and a query that now spends most of its life before touching a byte. If your only cost dashboard is scan volume, this incident is invisible until somebody complains about latency or a per-request line item.
> 
...
> **[vendor snapshot — verified 2026-09]** Target file size as a table property — and what the format cannot do for you
>
> Iceberg exposes the target directly: `write.target-file-size-bytes`, documented with a default of **536870912 — 512 MB**. There is a separate, smaller default for delete files, which is a hint about their expected lifetime rather than a preference.
> 
...
>
> Sources: https://iceberg.apache.org/docs/latest/configuration/ · https://iceberg.apache.org/docs/latest/maintenance/

---

_the file count ≡ problems you have already had_

- **a queue whose arrival rate exceeds its service rate** (There is no steady state and no consumer size that fixes it; the backlog grows without bound until you change the arrival rate.) ≡ **compaction against ingest** (Identical, and identically diagnosed: graph the two rates on one axis and look for the crossing, because the backlog only tells you how long ago it happened.)
- **an over-indexed OLTP table** (Every index accelerates one access path and taxes every single write. Nobody adds the eleventh index without an argument.) ≡ **an over-partitioned analytical table** (Every partition column accelerates predicates that name it and multiplies the file count for all writes. The eleventh partition column deserves the same argument.)
- **a directory holding a million tiny files** (The bytes are trivial and listing it is unusable, because the cost was never in the bytes — it was per directory entry.) ≡ **a table holding a million data files** (The planner is the listing, and it runs before every query. Per-entry costs do not care that each entry is small.)

---

**Q1. Planning is 61% of query time on a table with 1.4M small files. Which of these actually reduces the file count?**
   A. Partition more finely so each query touches fewer files
   B. Raise the batch interval to the largest value the freshness requirement permits, and size compaction capacity against the resulting creation rate — the interval is the input variable and the file count is the consequence
   C. Scale up query compute so the planning phase completes faster
   D. Disable statistics collection on ingest so there is less metadata per file
   Answer: B — File count is commits × partitions × writers, so the only levers are those three factors, and the commit interval is usually the one with the most headroom against its actual requirement. Finer partitioning is the trap: it reduces files per query only when the predicate names the partition key, and it multiplies total file count unconditionally — and partition boundaries then block compaction from merging across them, turning a backlog into a permanent floor. Scaling query compute pays the planning cost faster rather than removing it, and dropping statistics removes the pruning that makes the surviving files cheap to skip.

**Q2. A team ingests 25 GB/day and wants to partition by day and tenant_id (200 tenants) so tenant-scoped queries prune well. They target 512 MB files. What should you tell them?**
   A. Fine — 512 MB is the format default and compaction will reach it
   B. 200 partitions a day over 25 GB/day is 125 MB per partition per day, so 512 MB is unreachable inside a daily boundary no matter how compaction is tuned; either widen the partition to just day and put tenant_id in the sort key, or accept 125 MB files and say so in the design
   C. Add hourly partitioning as well, so compaction has smaller units to work with
   D. Increase the target to 1 GB so compaction merges more aggressively
   Answer: B — Divide before deciding: 25 GB ÷ 200 = 125 MB of data per partition per day, and a file cannot span two partitions, so 125 MB is the ceiling — the target is not a promise the format can keep. The fix is to move the high-cardinality column from the partition key to the sort key, which prunes inside files without multiplying them. Hourly partitioning divides the same data by another 24 and makes it strictly worse; raising the target changes a number nothing can reach.

**Q3. You are on a consumption pricing shape billed per byte scanned. You propose a week of work to cut the file count from 1.4M to 40k. Finance asks what it saves. What is the honest answer?**
   A. Roughly a 35× reduction in the query bill, since the file count falls 35×
   B. Close to nothing on the per-byte line, because bytes scanned barely changes — the saving is in latency, compute-seconds and per-request charges, plus the compaction rewrite volume it removes; so the case has to be made in those currencies and in the ad-hoc queries that are currently unusable
   C. Nothing measurable at all, so the work should be deprioritised
   D. A 61% reduction in cost, matching the planning share of query time
   Answer: B — This is the trap C0.L5 set up: the optimisation and the pricing shape have to match, or you will be technically right and commercially irrelevant. File count barely touches bytes scanned, so a per-byte-scanned bill hardly moves. The real savings are elsewhere and are still real — request counts, compute time, the ~1.4 TB/day of compaction rewrite, and the analyst queries that currently spend most of their life planning. Quoting the 61% planning share as a cost reduction is the specific mistake: it is a share of time, on a shape that does not bill time.
