# C1.L5 — Compute Without Decoding

_Track C1: Encodings · ~15 min · columnspaces_

> Summing a run-length encoded column is one multiplication. Decoding it first is 300,000 additions for the same answer.
Every codec in this track has been justified by bytes: fewer bytes on storage, fewer bytes across a fabric. That is the first payoff and the obvious one. There are two more, and the last one changes how operators are written.
...
_fig 1 — the same filter, two places to do the work_

Components: WHERE region = 'EMEA' (2,000,000 rows · 8 distinct values) · decode first (expand codes back to strings) · stay encoded (the dictionary is 8 entries) · 2,000,000 string compares (pointer chase + memcmp each) · 8 string compares ('EMEA' → code 3) · then code == 3 (packed ints · vectorises) · or skip the block entirely (code absent from the dictionary) · identical answer (the results must agree — graded)

Steps:
1. One equality predicate over two million rows on a column with eight distinct values. The bytes on storage are identical in both branches below — the only difference is where the comparison happens.
2. The eager path decodes the column back into strings and then compares each one. Two million pointer chases and memcmps, plus the memory traffic to materialise strings the engine had already compressed away.
3. The lazy path compares the literal against the dictionary instead — eight string comparisons, once — and learns that EMEA is code 3. The expensive operation just happened 250,000 times less often.
4. The filter over the column is now an integer comparison against a densely packed array, which is the shape a compiler will vectorise. This is where encoding stops being a storage technique and becomes an execution one.
5. And there is a shortcut the range-based zone map cannot offer: a dictionary is the complete set of values present in the block, so if the literal is absent the block cannot match and the data pages are never read. For equality, one-sided error becomes no error.
6. Both paths must return the identical answer, and that equivalence is what makes the optimisation safe rather than clever. Forge lab 01 grades it directly: the compressed path and the decode-then-execute path have to agree on every seeded batch.

---

```python
# decode first — correct, and it does work proportional to rows
...
- **300,000 → 1** — operations to sum a constant column (One run means count and sum are arithmetic on the run header. Work proportional to runs, not to rows.)
- **2,000,000 → 8** — string comparisons for an equality filter (Compare against the dictionary once per distinct value, then filter integer codes. The expensive operation happens 8 times.)
- **3** — payoffs from one encoding decision (I/O, memory bandwidth, and CPU. Only the third needs operators that never decode — which is why it is the one most often skipped.)
- **0** — answers the fast path may change (Graded as an equivalence in forge lab 01: the compressed path must agree with decode-then-execute on every seeded batch.)

---

> **[vendor snapshot — verified 2026-09]** Encoded chunks, and what a shared-everything design does with them
>
> Two claims to keep apart, because they are usually blurred.
> 
...
>
> Sources: https://support.vastdata.com/hc/en-us/articles/11910669291548-VAST-Database-Overview · https://www.vastdata.com/features/transactional-and-analytical-support · https://parquet.apache.org/docs/file-format/data-pages/encodings/

---

_compute-on-encoded ≡ laziness you already exploit_

- **predicate pushdown** (Move the filter to where the data is instead of moving the data to the filter. The win is the work you never do.) ≡ **evaluating against the dictionary** (Move the comparison to where the distinct values are — eight of them — instead of to where the rows are.)
- **a materialized view you did not need** (Computing eagerly is only cheaper if the result gets used. Otherwise it is work plus storage plus staleness.) ≡ **decoding eagerly** (Materialising values the operator could have consumed encoded is work plus allocation plus memory traffic, for nothing.)
- **a fast path in a hot loop** (Every specialised path must return exactly what the general path returns, or you have shipped a bug that only appears under load.) ≡ **the compressed execution path** (Graded as an equivalence for the same reason: a fast path that disagrees with the reference is a correctness bug wearing a benchmark.)

---

**Q1. A column of 300,000 rows holds one value throughout, run-length encoded as a single run. What does SUM over that column cost on the encoded form?**
   A. 300,000 additions — the values must be visited to be summed
   B. One multiplication: value × count, taken from the run header, with no array materialised
   C. It cannot be computed without decoding, since SUM needs individual values
   D. Two operations: one to decode the run and one to sum it
   Answer: B — A sum of a constant is the constant times the count, and both are in the run header. Work becomes proportional to the number of runs rather than the number of rows, which is unbounded upside on a well-clustered column — and it is the clearest case of encoding reducing CPU rather than just bytes.

**Q2. Why does filtering on a dictionary-encoded column turn 2,000,000 string comparisons into 8?**
   A. The dictionary is sorted, so a binary search finds the matching rows
   B. The literal is compared against the dictionary's distinct values once to obtain its code, and the filter over the column is then an integer comparison on packed codes
   C. The compressor removes duplicate rows before the filter runs
   D. Because string comparison is vectorised in modern CPUs
   Answer: B — The expensive operation — string comparison — is moved from once per row to once per distinct value, and what remains is an integer comparison over a dense array, which is what actually vectorises. It also enables an exact block skip: a dictionary is the complete set of values present, so an absent literal means the block cannot match.

**Q3. An engineer proposes joining two tables on dictionary codes instead of decoded strings, "since codes are just integers and integers compare faster". What is wrong?**
   A. Nothing — this is the standard way to execute a join on encoded columns
   B. Dictionaries are per block, so the same string can have different codes in different blocks: the join needs a shared global dictionary or a translation step, and maintaining a global dictionary under concurrent writes is its own cost
   C. Joins cannot use integer keys, only strings
   D. It works but produces the wrong row counts when nulls are present
   Answer: B — Codes are local identifiers, not values, so comparing them across blocks compares two different namespaces and silently returns wrong results. Recovering the optimisation needs either a global dictionary — expensive to maintain as data arrives — or a translation on one side. This is the main limit on how far execute-on-compressed composes, and it is why the equivalence with the decoded path has to be graded rather than assumed.

---

The canonical reference is **Abadi, Madden and Ferreira, "Integrating Compression and Execution in Column-Oriented Database Systems" (SIGMOD 2006)**. It is the paper that argued compression should not be a storage-layer detail hidden behind a decode step, and it introduced the framing this lesson uses: operators that accept encoded blocks, with the decoded path as the correctness reference. Read section by section against what you implemented in forge lab 01 — the correspondence is close, and the paper's discussion of *when* the encoded path loses is as valuable as when it wins.
