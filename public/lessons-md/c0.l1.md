...
- **96%** — row groups skipped unread (Measured in a browser, not assumed: 1 of 25 row groups read for a seven-day window on a table clustered by time. Blocks whose max predates the predicate are never opened.)
- **1,103×** — the factors multiplied (Measured in the lab: 19x projection times 58x pruning. They act on different terms of the same product, which is why they multiply rather than add.)
