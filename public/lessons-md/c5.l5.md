# C5.L5 — The Update Path Decision

_Track C5: The Write Path · ~20 min · columnspaces_

> A compaction policy with no stated write-amplification number is an unbounded background bill with a cron entry. This is the page that fixes that: five numbers, one named loser, and the weakness of your own model said before the room finds it.
This is the lesson where C5 becomes a document somebody signs. It is one page, it has five numbers, and each one maps to a check the compaction desk grades: `read_amp_bounded`, `write_amp_budget`, `expiry_stated`, `storage_stable`, `cost_counted`.
...
## The five line items, and the arithmetic each one needs
...
> **[warning]** Every number on the page above comes from a model, and a model that hides its exclusions is a sales document. Three costs are missing, and all three run the same direction — upward.
> 
...
> **[vendor snapshot — verified 2026-09]** A platform that claims this line item away — and the five measurements that would test it
>
> Everything above is a bill created by one physical assumption: **change is expressed by rewriting large immutable files.** So the most interesting vendor claim in this track is the one that attacks the assumption rather than the bill.
> 
...
>
> Sources: https://www.vastdata.com/features/transactional-and-analytical-support · https://www.vastdata.com/blog/the-data-lake-dilemma · https://iceberg.apache.org/docs/latest/configuration/

---

_fig 5 — five numbers, one named loser, and the caveat that changes which policy you ship_

Components: 1 · trigger (every 16 uncompacted files) · 2 · target size (256Ki rows ≈ 16 MiB sealed) · 3 · expiry window (24 commits · costs 1.09× storage) · 4 · read ceiling 1.02× — holding at 1.015× (data bytes and metadata bytes quoted separately) · 5 · write budget 2.60× — spending 2.11× (312.6 MiB ingested, 347.4 MiB rewritten, 17 compactions) · the line item, in counts: bytes rewritten/day · % of table/day · created vs merged per hour · requests added · files live · held ÷ live (C3.L5's template — about 1.4 TB/day on a 9 TB table, roughly 15% of the table daily, producing zero rows anybody asked for) · the page the compaction desk grades (read_amp_bounded · write_amp_budget · expiry_stated · storage_stable · cost_counted) · the caveat, said first (this model never rewrites a sealed file, so 2.11× is a FLOOR) · so the slack goes on the write axis, and the monitors are stranded rows, backlog slope, and compactions attempted vs committed (the read-better policy at 2.56× against a 2.60× budget had no room for a cost we already knew we had not counted)

Steps:
1. Two dials chosen from a priced grid rather than from taste: compact every sixteen uncompacted files, seal base files at 256Ki rows, which is one of only four policies in thirty that satisfies both constraints at this write rate.
2. A retention window is the third number and it is a risk decision rather than a storage setting: twenty-four commits of superseded files costs about 1.09× live storage here, and after the window recovery means restore-from-backup.
3. Then the pair that makes it a policy instead of a preference — a read ceiling with its measurement method, and a write budget expressed as bytes rewritten per byte ingested, both derived from the same run.
4. Costed as counts and never as a clock or a price: bytes rewritten per day, that as a share of the table, creation rate against merge rate, requests added, and the pricing shape all of it lands on.
5. Five numbers make the page the compaction desk grades, and it grades in bands — generous to a model with a derivation behind it, unforgiving of a round number that arrived by itself.
6. And then the sentence that earns the room: the model behind these figures never rewrites a sealed file, so its write amplification is a floor rather than a forecast, and a real tombstone-purging pass will raise it.
7. Which is why the slack sits on the write axis and why the read-better policy was rejected: 2.56× against a 2.60× budget leaves no room for a cost you have already admitted you did not count.

---

- **2.11×** — write amplification shipped, against a 2.60× budget (312.6 MiB ingested and 347.4 MiB rewritten by 17 compactions, so every ingested byte is written 1.11 more times. The 0.49× of headroom is deliberate, because the model excludes rewriting sealed files.)
- **1.015×** — read amplification, against a 1.02× ceiling (Averaged over a read after every commit rather than measured at the end of the run, so the figure does not grade where the last compaction happened to fall.)
- **1.09×** — storage held ÷ live at a 24-commit retention window (25.6 MiB of superseded bytes against 335.8 MiB live. C3.L4 measured over 1.8× for one retained snapshot on a coarse layout, so this multiple is a property of file size and rewrite rate rather than a constant.)
- **~15%/day** — of the table rewritten in C3.L5's worked example (About 1.4 TB/day on a 9 TB table, producing no rows a user asked for. Above roughly 20% you are rewriting the whole table every five days to service an ingest configuration.)

---

> **[analogy]** This is the C5 version of the paragraph C2.L6 taught, and it is what the compaction desk and the platform runbook are both asking for. Ninety seconds, every number redoable:
> 
...
_a compaction policy ≡ documents you already defend_

- **an error budget** (A stated allowance, a measurement everyone accepts, and an agreed consequence when it is spent. Without the allowance, every incident is a negotiation from zero.) ≡ **the write-amplification budget** (Identical function: 2.11× spent against 2.60× allowed means an ingest change that pushes it to 2.8× is a visible, arguable event rather than a slow rise nobody attributed.)
- **a backup retention policy** (Everyone understands that retention costs storage and that deleting a backup removes a recovery option. Nobody calls it a storage optimisation, and the window has an owner.) ≡ **the expiry window** (The same document, with one extra sting: time travel lives in the same metadata tree it would have to recover from, so an RPO that leans on it has one failure domain where you believed you had two — which is what dr-desk grades.)
- **a capacity plan with a named first bottleneck** (The credible part is not the forecast. It is knowing which limit binds first and roughly when, so that monitoring points at the right graph.) ≡ **stranded rows and backlog slope** (The two leading indicators of this policy failing. Storage growth and read amplification are the lagging ones, and the invoice is worse than both — the same argument C2.L6 made for clustering depth.)

---

**Q1. Two policies both satisfy your constraints. Policy A: read 1.007×, write 2.56×, 349 stranded tombstoned rows, 402 MiB held. Policy B: read 1.015×, write 2.11×, 0 stranded, 366 MiB held. Your write budget is 2.60× and your read ceiling is 1.02×. Which ships, and on what argument?**
   A. A, because read amplification is what users experience and it is measurably better, while both policies satisfy the write budget
   B. B, because the model behind these figures never rewrites a sealed file to purge tombstones, so its write amplification is a floor and A has only 0.04× of headroom against a cost we know is uncounted. B keeps 0.49×, strands nothing and holds 36 MiB less — and the loser is stated: 0.008× of read amplification and fourteen more files opened per read
   C. A, and add a separate tombstone-purging job later if stranded rows become a problem, since 349 rows is negligible today
   D. Either — both are inside both constraints, so the choice is operational preference and not worth a paragraph in the document
   Answer: B — This is the lesson's central move: the caveat is not decoration, it decides the choice. A model that excludes rewriting sealed files reports write amplification as a floor, so a policy 0.04× under the budget is effectively already over it once the excluded pass is funded, while a policy 0.49× under has room. Preferring A on read amplification treats the two numbers as equally trustworthy when one of them is known to be an underestimate. Deferring a purge job is defensible only with a trigger and a budget attached — "later if it becomes a problem" is how stranded rows grow into the case C5.L3 measured, where no trigger setting can recover the read ceiling. And calling it preference skips the sentence that makes the document credible: every layout and every policy has a loser, and naming it is the deliverable.

**Q2. A platform team reports: "compaction runs nightly and read amplification is fine." What is missing, and why does its absence show up as a cost surprise rather than an incident?**
   A. A latency SLO for the compaction job, since a nightly job that overruns its window will collide with the morning query peak
